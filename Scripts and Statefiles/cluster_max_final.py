...
# load state
LoadState(script_dir+'max_resub.pvsm', LoadStateDataFileOptions='Choose File Names',
    DataDirectory=script_dir,
    scalar_field_vtkFileNames=data_path)

geodesicClustering = FindSource('Geodesic Clustering')

os.mkdir(data_dir+"clustered max")

SaveData(data_dir+'clustered max/clustered_max.vtu', proxy=geodesicClustering, Writetimestepsasfileseries=1)
